...
// Paths generated via topojson-client + d3-geo — do not edit by hand.
//
// ViewBox "-50 100 620 430":
//   top    y=100  ≈ 60°N  (just above Stockholm y=124, Oslo y=111)
//   bottom y=530  ≈ 35°N  (southern Spain / Gibraltar)
//   Lapland / far-north Scandinavia cropped; all 12 supported countries visible.
...
interface CountryShape {
  ...
  ly: number;
  /**
   * 'always'  — label visible at rest (large countries with enough room)
   * 'hover'   — label shown only on hover/focus (small / dense countries)
   */
  label: "always" | "hover";
}
...
const COUNTRY_SHAPES: CountryShape[] = [
  // ── Central / Northern Europe ─────────────────────────────────────────────
  {
    ...
    lx: 352, ly: 165,   // southern-Sweden centroid; northern portion is cropped
    label: "always",
  },
  {
    id: "dk",
...
export function EuropeMap({ supportedIds, onCountryClick, className }: EuropeMapProps) {
  ...
  return (
    // ViewBox starts at y=100 (≈60°N, just above Stockholm/Oslo) so the
    // far-north is cropped out and useful countries fill the space.
    <svg
      viewBox="-50 100 620 430"
    ...
    >
      <defs>
        <filter id="emap-focus" x="-10%" y="-10%" width="120%" height="120%">
          <feDropShadow dx="0" dy="0" stdDeviation="3"
            floodColor="hsl(var(--accent))" floodOpacity="0.6" />
        </filter>
      </defs>

      {COUNTRY_SHAPES.map((shape) => {
        const isSupported = supportedIds.has(shape.id);
        const isActive    = hovered === shape.id || focused === shape.id;
        const isFocused   = focused === shape.id;

        // Colors — all from design-system tokens, no hardcoded values
        // Default: primary (matte ink #0b0f0d)
        // Active:  accent  (forest green #1f5f3f)
        const fill   = isSupported
          ? (isActive ? "hsl(var(--accent))" : "hsl(var(--primary))")
          : "hsl(var(--muted))";
        // Stroke separates countries; lighter on active for legibility
        const stroke = isActive
          ? "hsl(var(--accent-foreground))"   // white
          : "hsl(var(--background))";          // page background (off-white)
        // Label text is always the foreground for its fill color
        const labelFill = isActive
          ? "hsl(var(--accent-foreground))"    // white on green
          : "hsl(var(--primary-foreground))";  // white on ink
        ...
        return (
          ...
          >
            ...
            {/* Persistent label for large countries */}
            {isSupported && shape.label === "always" && (
              <text
                x={shape.lx}
                y={shape.ly}
                textAnchor="middle"
            ...
            {/* Tooltip-style label on hover for small / dense countries */}
            {isSupported && shape.label === "hover" && isActive && (
              <>
                {/* pill background for legibility over neighbour countries */}
                <rect
                  x={shape.lx - 24}
                  y={shape.ly - 22}
                  width={48}
