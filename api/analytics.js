...
// ── In-memory access token cache (lambda warm reuse) ────────────────────────
...
async function ga4(token, body) {
  ...
  const json = await res.json();
  if (!res.ok) throw new Error(`GA4: ${json.error?.message ?? res.statusText}`);
  return json;
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const [overview, pages, channels, funnel] = await Promise.all([
      ...
      // Quiz funnel — counts of each custom event
      ga4(token, {
        dateRanges,
    ...
    const num = (i) => parseFloat(ov[i]?.value ?? "0");

    const funnelCounts = Object.fromEntries(
      (funnel.rows ?? []).map((r) => [r.dimensionValues[0].value, parseInt(r.metricValues[0].value, 10)])
    );
